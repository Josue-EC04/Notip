...
async function run(){
 ...
 const win=new BrowserWindow({width:1050,height:760,show:false,webPreferences:{backgroundThrottling:false,preload:path.join(root,'preload.js'),contextIsolation:true,nodeIntegration:false}});
 ...
 await win.loadFile(path.join(root,'src/study/study.html'));
 await until(win,"document.querySelector('#inbox-list .empty') !== null");
 await win.webContents.executeJavaScript("document.getElementById('pause-ai').click()");
 await until(win,"document.getElementById('pause-ai').textContent.includes('Reanudar')");
 await win.webContents.executeJavaScript("document.getElementById('capture-text').value='Duda sobre subnetting y máscara /27';document.getElementById('capture-form').requestSubmit()");
 ...
 await win.webContents.executeJavaScript("document.querySelector('[data-tab=classes]').click();document.getElementById('class-name').value='Redes · Subnetting';document.getElementById('class-form').requestSubmit()");
 await until(win,"document.getElementById('class-context').textContent.includes('Redes')");
 for(const text of ['No entiendo cómo calcular equipos con máscara /27','Entregar ejercicios de subnetting']) {
  await win.webContents.executeJavaScript(`document.getElementById('capture-text').value=${JSON.stringify(text)};document.getElementById('capture-form').requestSubmit()`);await delay(100);
 }
 await win.webContents.executeJavaScript("document.querySelector('[data-tab=classes]').click();Array.from(document.querySelectorAll('#active-class button')).find(b=>b.textContent.includes('Terminar')).click()");
 ...
 await screenshot(win,'03-clase.png');
 assert.equal(db.getTasks().length,2);
 await win.webContents.executeJavaScript("document.querySelector('[data-tab=focus]').click()");await until(win,"document.querySelector('#effort-form')!==null");
 ...
 await win.webContents.executeJavaScript("document.querySelector('[data-tab=inbox]').click();document.getElementById('show-done').checked=true;document.getElementById('show-done').dispatchEvent(new Event('change'))");
 ...
 win.setSize(740,650);await delay(200);
 assert.equal(await win.webContents.executeJavaScript("document.querySelector('main').scrollWidth<=document.querySelector('main').clientWidth"),true);
